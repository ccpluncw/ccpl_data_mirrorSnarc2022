...
numDistLmeFormula.N <- as.formula("mRT~ wel + relFive +  (1 |sn/Numbers)")
snarcLmeFormula.N <- as.formula("dRT~ Numbers + relFive + (1|sn)")
numDistLmeFormula.M <- as.formula("mRT~ wel + relFive + simSelf + (1 |sn/Numbers)")
snarcLmeFormula.M <- as.formula("dRT~ Numbers + relFive + simSelf + (1|sn)")

numDistFormula.N <- as.formula("mRT ~ wel + relFive")
snarcFormula.N <- as.formula("dRT ~ Numbers + relFive ")
numDistFormula.M <- as.formula("mRT ~ wel + relFive + simSelf")
snarcFormula.M <- as.formula("dRT ~ Numbers + relFive + simSelf")
...
#get structure from experiment link
...
for(i in structs) {
  ...
  sink(outStatFileName, appendState)
    cat("\n\n\n#############################",i,"#############################","\n")
    cat("\n\n ############### Mixed Model Regression assessing Numerical Distance ###############\n\n")
    cat("\n ############ Full Welford Model ############n\n")
    cat("\n ##### summary output #####\n\n")
...
df.slopes.m <- NULL
for(i in subs) {
    struct <- unique(df.LR[df.LR$sn == i,"Structure"])
    order <- unique(df.LR[df.LR$sn == i,"order"])

    df.lr.tmp <- df.LR[df.LR$sn == i,]
    df.hand.tmp <- df.hand[df.hand$sn == i,]
    ...
    colnames(df.tmp.wel) <- paste("welford" ,original_cols,sep="_")
...
#############  Overall Snarc and Numerical Distance  ################
...
for(i in structs) {
  ...
  with(df.num[df.num$Structure == i,], plot(mRT~Numbers, main=paste(i, "Numerical Dist"), col="grey", ylim=c(minY,maxY), xlim=c(0,10),pch = 16,cex = cexSize))
  ...
  with(df.num.1, plot(mRT~Numbers, main=paste(i, "Numerical Dist"), col="grey", ylim=yLims.ND, xlim=c(0,10),pch = 16,cex = cexSize))
...
pdf("snarc boxplot.pdf", width = 10, height = 7)
  ...
  for(i in structs) {
    ...
    df.tmp.1 <- df.tmp[ , grepl( "snarc_" , names( df.tmp ) ) ]
    df.tmp.1 <-df.tmp.1 %>% rename_all(~stringr::str_replace(.,"^snarc_",""))
    boxplot(df.tmp.1, main = paste(i,"SNARC"),  ylim = yLim.Snc)
    abline(h=0, col = "grey")

    df.tmp.1 <- df.tmp[ , grepl( "welford_" , names( df.tmp ) ) ]
    df.tmp.1 <-df.tmp.1 %>% rename_all(~stringr::str_replace(.,"^welford_",""))
    boxplot(df.tmp.1, main = paste(i,"Welford"), ylim = yLim.Wel)
    abline(h=0, col = "grey")
  }
