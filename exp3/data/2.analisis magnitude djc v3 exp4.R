...
cvOutlierTrim <- 0.95
numTrials <- 0
removeSuccessiveSameTrials <- FALSE
takeResiduals <- FALSE
cexSize <- 1.5
...
#List files
raw.data = list.files(".",pattern=".csv",full.names=T, all.files=T)
...
#head.all$Structure <- ifelse(head.all$link == "https://www.testable.org/experiment/7726/817031/start" | head.all$link == "https://www.testable.org/experiment/7726/995587/start", "MIRROR", "NORMAL" )

head.all$link <- NULL

#Remove unnecessary columns
#dat.all <- subset(dat.all, PracticeTrials == "no",  select = - c(rowNo, type, stim1,stim2, stim3,presTime,ISI,responseWindow, feedback, feedbackTime,feedbackOptions,stimFormat,random,keyboard,cursor,presTime_ms,presTime_f,presTime_fDuration))
...
df <- df[df$trialInBlock > numTrials, ]
...
if(removeLearning) {
  subs <- unique (df.c1$sn)

  df.res <- NULL
  for (j in subs) {
      df.tmp <- df.c1[df.c1$sn == j, ]
      outList <- ch.rmLearningEffect(df.tmp, "trial","RT", "fitLM", "resRT")
      df.res <- ch.rbind (df.res, outList$data)
  }

  df.res$RT <- df.res$resRT
  df.final <- df.res
} else {
  df.final <- df.c1
}
...
if(removeSuccessiveSameTrials) {
  xx <- df.final[order(df.final$sn,df.final$trial),]
  x1 <- xx$Numbers
  x2 <- append(x1,0, after=0)
  x2 <- x2[-length(x2)]
  xx$NumbersOffset <- x2
  xx$remove <- ifelse(xx$Numbers == xx$NumbersOffset, TRUE, FALSE)
  df.final <- xx[xx$remove == FALSE, ]
}
...
df.hand$StructureEC <- ifelse(df.hand$Structure == "MIRROR", -1, 1)
...
for(i in structs) {
  ...
  #remove effect of welford and similarity relative to 5
  if(takeResiduals) {
    df.hand.tmp$res.RT <- resid(df.wel.lmer)
  } else {
...
df.hand <- df.hand.out
#mean RT by Number for each Subject
structs <- unique(df.LR$Structure)
subs <- unique(df.LR$sn)
pdf("all sub plots.pdf")
df.slopes.n <- NULL
df.slopes.m <- NULL
for(i in subs) {
  for(j in structs) {
    ...
    with(df.lr.tmp, plot(dRT~Numbers, main=paste(i,struct, "SNARC")))
    with(df.lr.tmp, points(predict(snarc.lm)~Numbers, pch=16))
    with(df.hand.tmp, plot(mRT~Numbers, main=paste(i,struct, "SimtToSelf")))
    with(df.hand.tmp, points(predict(numDist.lm)~Numbers, pch=16))
...
#############  Overall Snarc (dRT) and Similrity to Self (mRT)  ################
  ...
  df.LR.MN <- df.LR
  if(snarkLinear) {
    lmALL.MN <- lm(snarcFormula.M, data = df.LR.MN)
  } else {
    lmALL.MN <- ch.getLearningEffect(df.LR.MN, "Numbers", "dRT", a = 40, b = -1, c = 1)
  }
  ...
  df.num.lm <- lm(numDistFormula.M, data=df.num)

sink(outStatFileName, appendState)
  cat("\n\n\n ############### ALL CONDITIONS ###############n\n")
  cat("\n\n --------- SNARC (dRT) \n")
  print(summary(lmALL.MN))
  cat("\n\n --------- Similarity To Self (mRT) \n")
  print(summary(df.num.lm))
sink(NULL)


pdf("summary plots.pdf")
  op <- par(bty="n")
  minY <- min(df.LR.MN$dRT, predict(lmALL.MN)) - .1*abs(min(df.LR.MN$dRT, predict(lmALL.MN)))
  maxY <- 1.1*max(df.LR.MN$dRT, predict(lmALL.MN))
  with(df.LR.MN, plot(dRT~Numbers, main="SNARC_ALL_N-M", col="grey", ylim=c(minY,maxY), xlim=c(0,10),pch = 16,cex = cexSize))
  with(df.LR.MN, points(predict(lmALL.MN)~Numbers, pch = 16, col="black", cex = cexSize))

  minY <- min(df.num$mRT, predict(df.num.lm)) - .1*abs(min(df.num$mRT, predict(df.num.lm)))
  maxY <- 1.1*max(df.num$mRT, predict(df.num.lm))
  with(df.num, plot(mRT~Numbers, main="SimToSelf_ALL_N-M", col="grey", ylim=c(minY,maxY), xlim=c(0,10),pch = 16,cex = cexSize))
  with(df.num, points(predict(df.num.lm)~Numbers, pch = 16, col="black", cex = cexSize))
...
for(i in structs) {
  ...
  if(snarkLinear) {
    ...
    snarc.lm <- ch.getLearningEffect(df.tmp, "Numbers", "dRT", a = 40, b = -1, c = 1)
  }
...
dev.off()
 ...
 df.LR.a$dRT <- df.LR.a$Right - df.LR.a$Left
  #ANOVA ALL MAGNITUDE mRT (Hand2 x Magnitude2 x Structure2)
  Ano.2.NM.dRT <- anova_test(data = df.LR.a,
                            dv = dRT, wid = sn,
                            within = c(Magnitude,Structure))

  Ano.order.NM.dRT.1 <- anova_test(data = df.LR.a[df.LR.a$Structure == "MIRROR",],
                            dv = dRT, wid = sn,
                            within = c(Magnitude))

  Ano.order.NM.dRT.2 <- anova_test(data = df.LR.a[df.LR.a$Structure == "NORMAL",],
                            dv = dRT, wid = sn,
                            within = c(Magnitude))

  df.summary <- data.frame(df.LR.a %>% group_by(Structure,Magnitude) %>% summarize(mDRT = mean(dRT, na.rm=T), sdDRT = sd(dRT, na.rm=T)))
...
pdf("snarc boxplot.pdf", width = 10, height = 7)
  ...
  boxplot(df.slopes.m$snc_r,df.slopes.n$snc_r, main = "SNARC r",names=c("Mirror","Normal"))
  abline(h=0, col = "grey")
  boxplot(df.slopes.m$wel_r,df.slopes.n$wel_r, main = "simToSelf r",names=c("Mirror","Normal"))
  abline(h=0, col = "grey")
  boxplot(df.slopes.m$snc_r2,df.slopes.n$snc_r2, main = "SNARC r2",names=c("Mirror","Normal"))
  abline(h=0, col = "grey")
  boxplot(df.slopes.m$wel_r2,df.slopes.n$snc_r2, main = "simToSelf r2",names=c("Mirror","Normal"))
  abline(h=0, col = "grey")
  par(op)
  op <- par(bty="n")
  with(df.LR.a, boxplot(dRT ~ Magnitude*Structure ))
  abline(h=0, col = "grey")
  with(df.slopes.n, plot(snarc_Numbers~simToSelf_simSelf, main = "Normal"))
  norm.lm.ws <- with(df.slopes.n, lm(snarc_Numbers~simToSelf_simSelf))
  abline(norm.lm.ws)
  with(df.slopes.m, plot(snarc_Numbers~simToSelf_simSelf, main = "Mirror"))
  mirror.lm.ws <- with(df.slopes.m, lm(snarc_Numbers~simToSelf_simSelf))
  abline(mirror.lm.ws)
  with(df.slopes.m, hist(snarc_Numbers, breaks = 15, main = "MIRROR"))
  with(df.slopes.n, hist(snarc_Numbers, breaks = 15, main = "NORMAL"))
  with(df.slopes.m, plot(simToSelf_simSelf~wel_r, main = "Mirror and Normal"))
  with(df.slopes.n, points(simToSelf_simSelf~wel_r, col="red"))
  cv.lm.n <- with(df.slopes.n, lm(snarc_Numbers~cv))
  cv.lm.m <- with(df.slopes.m, lm(snarc_Numbers~cv))
  with(df.slopes.n, plot(snarc_Numbers~cv, main = "snarc x CV - Normal"))
  abline(cv.lm.n)
  with(df.slopes.m, plot(snarc_Numbers~cv, main = "snarc x CV - Mirror"))
  abline(cv.lm.m)

  wel.cv.m <- with(df.slopes.m, lm(simToSelf_simSelf~cv))
  with(df.slopes.m, plot(simToSelf_simSelf~cv, main = "simToSelf x CV - MIRROR"))
  abline(wel.cv.m)

  wel.cv.n <- with(df.slopes.n, lm(simToSelf_simSelf~cv))
  with(df.slopes.n, plot(simToSelf_simSelf~cv, main = "simToSelf x CV - NORMAL"))
  abline(wel.cv.n)
dev.off()
  ...
  sink(outStatFileName, appendState)
    cat("\n\n\n ############### ANOVA assessing Snark ###############\n\n")
    cat("\n ##### All Effects #####\n\n")
    print(get_anova_table(Ano.2.NM.dRT))
    cat("\n ##### Struction: Mirror #####\n\n")
    print(get_anova_table(Ano.order.NM.dRT.1))
    cat("\n ##### Struction: Normal #####\n\n")
    print(get_anova_table(Ano.order.NM.dRT.2))
    cat("\n ##### summary means #####\n\n")
    print(df.summary)
    cat("\n\n\n ############### Regression assessing Snark ~ simToSelf###############\n\n")
    cat("\n ##### Structure: Mirror #####\n\n")
    print(summary(mirror.lm.ws))
    cat("\n ##### Structure: Normal #####\n\n")
    print(summary(norm.lm.ws))
    cat("\n\n\n ############### Regression snarc_r ~ cv (snarc) ###############\n\n")
    cat("\n ##### Structure: Mirror #####\n\n")
    print(summary(cv.lm.m))
    cat("\n ##### Structure: Normal #####\n\n")
    print(summary(cv.lm.n))

  sink(NULL)


  sink(outStatFileName, appendState)
    cat("\n\n\n ############### T-Tests of all Sn fits to assess whether slopes are different than 0 ###############\n\n")
    ...
    for(i in structs) {
      ...
      df.tmp.1 <- data.frame(df.tmp[ , grepl( "snarc_" , names( df.tmp ) ) ])
      cat("\n\n ############### SNARC ", i, " ###############\n\n")
      for(j in 1:length(df.tmp.1)) {
        col.tmp <- colnames (df.tmp.1)[j]
        ttest.tmp <- t.test(df.tmp.1[[col.tmp]], mu=0)
        cat("\n\n ########", col.tmp, " ########\n")
        print(ttest.tmp)
      }
      cat("\n\n ############### simToSelf ", i, " ###############\n\n")
