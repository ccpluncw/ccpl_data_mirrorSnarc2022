...
for(i in structs) {
  ...
  ### switched optimization function because of convergence issues
  df.snarc.lmer <- lmer(snarcLmeFormula, control = lmerControl(check.conv.grad = .makeCC("warning", tol = 2e-2, relTol = NULL),optimizer ="nlminbwrap"), data=df.LR.tmp)
